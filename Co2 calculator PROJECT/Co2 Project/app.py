...
with open("./style/style.css", "r") as style:
    ...
    try:
        css = f"""<style>{css_content.format(background=background, icon2=icon2, icon3=icon3)}</style>"""
    except KeyError:
        st.error("CSS placeholders for 'background', 'icon2', or 'icon3' are missing.")
...
def component():
    ...
    grocery_bill = tab5.slider('Monthly grocery spending in rupees', 0, 5000, 0)
...
tab_result.markdown(f"""You owe nature <b>{tree_count}</b> tree{'s' if tree_count > 1 else ''} monthly. <br> {f"<a href='' id = 'button-17' class='button-17' role='button'> 🌳 save trees🌳</a>" if tree_count > 0 else ""}""",  unsafe_allow_html=True)
